...
console.log(devolverCervezas(beers, nivel));

function devolverCervezas(beers, nivel) {
  let beersOkey = beers.filter((beer) => beer.abv < nivel);
  beersOkey = beersOkey.map((beer) => {
...
console.log(masAlcoholicas(beers));

function masAlcoholicas (beers = []){
    beers = beers.sort((a,b) => b.abv - a.abv);
    let tenBeers = [];
    for (let i = 0; i < 10; i++) {
...
function ordenar(beers,propiedad,booleano) {
    if(booleano == true){
        beers = beers.sort((a,b) => a[propiedad] - b[propiedad]);
        let beersOkey = [];
        for (let i = 0; i < 10; i++) {
            beersOkey.push(beers[i]);
        }
        return beersOkey;
    }else{
        beers = beers.sort((a,b) => b[propiedad] - a[propiedad]);
        let beersOkey = [];
        for (let i = 0; i < 10; i++) {
            beersOkey.push(beers[i]);
        }
...
pintarCervezas(beers);

function pintarCervezas(beers) {
    ...
    beers.forEach(beer => {
        tbody.innerHTML += `
        <tr><td class="border border-dark">${beer.name}</td><td class="border border-dark">${beer.abv == null?"N/A":beer.abv}</td><td class="border border-dark">${beer.ibu == null?"N/A":beer.ibu}</td></tr>
        `;
    });
